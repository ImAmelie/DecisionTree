...
private:
    ...
    inline float Gini(const std::vector<T> &set);
    inline float Gini(const std::vector<std::pair<ArgType, RT>> &set);
    ...
    for (int i = 0; i < curArgs.size(); ++i) {
        ...
        if (argTypes.at(curIndex) == BestArgType::Discrete) {
            ...
            for (const auto &v : attrs) {
                ...
                sum += std::pow(float(attr_v_set.size()) / set.size(), 2);
            }
            t_max = 1 - sum;
            if (t_max < gini) {
                gini = t_max;
            ...
            for (ArgType t : tSet) {
                ...
                float gini_t = (((ArgType)minus.size() / a.size()) * Gini(minus)) + (((ArgType)plus.size() / a.size()) * Gini(plus));
                if (gini_t < t_max) {
                    t_max = gini_t;
                    tmpPoint = t;
                }
...
template<typename T, typename RT, RT &(*F)(const T &), int ArgNum, typename GetArgFun>
float Model<T, RT, F, ArgNum, GetArgFun>::Gini(const std::vector<std::pair<ArgType, RT>> &set) {
    // 获得 Gini(D)
    std::map<RT, int> types;
    for (const auto &v : set) {
        ++types[v.second];
